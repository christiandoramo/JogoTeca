...
public class CRUDJogos extends AccessAreaController implements Initializable {
	...
	protected void buscarRemoverPorId() {
		AccessAreaController.searchGameById(gc, campoRemoverId, listaRemover, destroyLog);
		modoRemocao = "id";
	}

	@FXML
	protected void buscarRemoverPorName() {
		AccessAreaController.searchGameByNome(gc, campoRemoverNome, listaRemover, destroyLog);
		modoRemocao = "name";
	}

	@FXML
	protected void buscarAtualizarPorId() {
		AccessAreaController.searchGameById(gc, CampoAtualizarPorId, JogoAAtualizar, updateLog);
		modoAtualizacao = "id";
	}

	@FXML
	protected void buscarAtualizarPorName() {
		AccessAreaController.searchGameByNome(gc, CampoAtualizarPorNome, JogoAAtualizar, updateLog);
...
//(String name, LocalDate releaseDate, Genre genre, String description, String publicadora, String desenvolvedora, Double price, String imageURL, String videoUrl, List<String> imagesUrl) throws ElementAlreadyExistsException, ElementWithSameNameExistsException {
	...
	protected void searchGameByIdentificador() {
		AccessAreaController.searchGameById(gc, CampoBuscarId, listaJogos, readLog);
	}

	@FXML
	protected void searchGameByNome() {
		AccessAreaController.searchGameByNome(gc, CampoBuscarNome, listaJogos, readLog);
	}
		...
		if (genero != null) {
			try {
				...
				if (!gamesAchados.isEmpty()) {
					AccessAreaController.mostraGamesAchados(listaJogos, gamesAchados);
	...
	protected void searchTodos() {
		try {
			...
			if (!allGames.isEmpty()) {
				AccessAreaController.mostraGamesAchados(listaJogos, allGames);
				readLog.setVisible(false);
			} else
	...
	public void initialize(URL url, ResourceBundle rb) {
		AccessAreaController.desabilitarDatasFuturas(releaseDate);
		AccessAreaController.desabilitarDatasFuturas(CampoTrocaReleaseDate);
		AccessAreaController.preencheMenuGeneros(genres);
		AccessAreaController.preencheMenuGeneros(CampoBuscarGenero);
		AccessAreaController.preencheMenuGeneros(CampoTrocaGenero);
		AccessAreaController.controlaDouble(price);
		AccessAreaController.controlaInteiro(campoRemoverId);
		AccessAreaController.controlaInteiro(CampoBuscarId);
	}
